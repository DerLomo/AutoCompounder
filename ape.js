...
const ApeRouterV2 = new web3.eth.Contract(swapABI,swapContract)
//Token contract's addresses
const wmaticContract= "0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270"
const maticContract = "0x8dF3aad3a84da6b69A4DA8aeC3eA40d9091B2Ac4"
const bananaContract= "0x5d47bAbA0d66083C52009271faF3F50DCc01023C"
...
async function harvest() {
	...
	const asktx = await MiniApeV2.methods.harvest(farmid,wallet.address).send(
    	{
    	    from : wallet.address,
    	    gas: 200000,
    	    gasPrice: gasPrice
...
async function swapBfM(amountIn) {
	...
	const swaping =await ApeRouterV2.methods.swapExactTokensForETH(
		...
		[bananaContract,wethContract,tethContract,wmaticContract],
		wallet.address,
...
async function liquidity(amountTokenDesired,amountETHMin) {
	...
	const addliquiditytx =await ApeRouterV2.methods.addLiquidityETH(
		bananaContract,
		web3.utils.toHex(amountTokenDesired),
		web3.utils.toHex(amountTokenMin),
		web3.utils.toHex(amountETHMin),
		wallet.address,
		web3.utils.toHex(Math.round(Date.now()/1000)+60*20),
		).send(
		{
		...
		console.log(`deposit status: ${addliquiditytx.status}`)
