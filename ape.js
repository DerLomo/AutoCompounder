...
//input parameters stuff
program.option('-p, --pair <type>', 'pair tocken to farm');
program.parse(process.argv);
const options = program.opts();
if (!options.pair) console.log("Pleas insert the token you want to pair with Matic");
let rawdata = fs.readFileSync('addresses.json');
let address_book = JSON.parse(rawdata);
//Farm's data
const farmid = address_book[0][options.pair][2]
...
const tokenContract = address_book[0][options.pair][0]
const PairContract	= address_book[0][options.pair][1]
...
async function swapBfM(amountIn,data) {
...
async function liquidity(amountTokenDesired,amountETHMin,data) {
...
async function deposit(pid,LPamount,data) {
	const gasPrice = await web3.eth.getGasPrice()
...
async function AutoCompound(){
    ...
    if(web3.utils.fromWei(curBanana.toString(),'ether') > txCost*6) {
    	console.log('Harvesting')
        const reclaim=await harvest()
        console.log('Swaping')
        const swap =await swapBfM(half,reclaim)
        console.log('Adding liquidity')
        const LPs= await liquidity(half,await pricing(half,tokenContract,wmaticContract),swap)
        console.log('Depositing LP tokens')
        const depositar = await deposit(farmid,await pricing(curBanana,wmaticContract,PairContract,LPs))
        console.log("Done")
    }
    else{
